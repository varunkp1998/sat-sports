...
// Serve profile pictures specifically if they are in a subfolder
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    // Check if the request is for a check-in or a profile
    if (req.path.includes('checkin')) {
      cb(null, UPLOADS_ROOT); // Or a specific CHECKINS_DIR if you define one
    } else {
  ...
  filename: (req, file, cb) => {
    ...
    const prefix = req.path.includes('checkin') ? 'checkin' : 'coach';
...
app.post("/api/coach/checkin/photo", upload.single('photo'), async (req, res) => {
  try {
    const { coachId, sessionId, locationId } = req.body;
    
    // Use the filename generated by your existing storage logic
    const photoName = req.file ? req.file.filename : null;

    if (!coachId || !sessionId || !photoName) {
      return res.status(400).json({ message: "Missing required fields or photo" });
    }
    ...
    if (existing.length > 0) {
      if (existing[0].checkout_time) return res.status(400).json({ message: "Session already completed" });
    ...
    // 3. INSERT (Method is marked as 'PHOTO')
    await db.query(
      `INSERT INTO coach_checkins
       (coach_id, session_id, location_id, lat, lng, checkin_time, is_late, verification_photo, method)
       VALUES (?, ?, ?, 0, 0, CONVERT_TZ(NOW(), '+00:00', '+05:30'), ?, ?, 'PHOTO')`,
      [coachId, sessionId, locationId, isLate, photoName]
    );
