...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    // Ensure this matches the static path above
    const dir = path.join(__dirname, 'uploads/profiles');
    cb(null, dir);
  },
  filename: (req, file, cb) => {
    const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
    // CRITICAL: Ensure path.extname(file.originalname) is included
...
app.post("/api/coach/upload-photo", upload.single('photo'), async (req, res) => {
  try {
    ...
    // Ensure the URL saved to DB includes the filename generated by multer
    const photoUrl = `/uploads/profiles/${req.file.filename}`; 

    await db.query("UPDATE coaches SET photo = ? WHERE user_id = ?", [photoUrl, userId]);
